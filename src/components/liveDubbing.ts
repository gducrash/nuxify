...
import { getVideoLiveCaptions, getLiveDubbingType, playLiveDubbing, setupFadeCssAnim, setupNuxOverlay, getYtVideoId, onElement } from './util';
...
export async function setupLiveDubbing (settings: ExtensionSettings) {
    ...
    function setupVideo (video: HTMLVideoElement) {
        if (!video || video.classList.contains('nuxified')) return;
        video.classList.add('nuxified');
    ...
    if (video) {
        ...
        onElement('.html5-main-video', elem => {
            setupVideo(elem as HTMLVideoElement);
        }, {
            childList: true,
            subtree: true,
        });
